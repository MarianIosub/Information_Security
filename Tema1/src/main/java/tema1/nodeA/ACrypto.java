...
public class ACrypto {
    ...
    public static byte[] chooseEncryption(byte[] text, byte[] key, byte[] IV, String operationType) throws NoSuchPaddingException, IllegalBlockSizeException, NoSuchAlgorithmException, BadPaddingException, InvalidKeyException {
        if (operationType.equals("ECB")) {
            return ECBEncrypt(text, key);
        } else {
            return OFBEncrypt(text, key, IV);
        }
    }
    ...
    private static byte[] ECBEncrypt(byte[] text, byte[] key) throws NoSuchPaddingException, IllegalBlockSizeException,
        ...
        while (contor != bytes.length / 16) {
            ...
            byte[] encryptedMessage = encryptMessage(cipherBlock, key);
    ...
    private static byte[] OFBEncrypt(byte[] text, byte[] key, byte[] iv) throws NoSuchPaddingException, IllegalBlockSizeException, NoSuchAlgorithmException, BadPaddingException, InvalidKeyException {
        List<Byte> myList = new ArrayList<>();
        byte[] cipherBlock = new byte[16];
        int contor = 0;
        int len = 16;
